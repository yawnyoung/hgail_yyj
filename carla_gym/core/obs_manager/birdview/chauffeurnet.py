...
class ObsManager(ObsManagerBase):
    ...
    def get_observation(self):
        ...
        def is_within_distance(w):
            ...
            c_ev = (
                abs(ev_loc.x - w.location.x) < 1.0
                and abs(ev_loc.y - w.location.y) < 1.0
            )
            return c_distance and (not c_ev)

        # FIXME: align with the carla14 API
        all_vehicles = self._world.get_actors().filter("vehicle.*")
        vehicle_bbox_list = [v.bounding_box for v in all_vehicles]
        all_pedestrians = self._world.get_actors().filter("pedestrian.*")
        walker_bbox_list = [p.bounding_box for p in all_pedestrians]
