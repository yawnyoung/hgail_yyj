...
class LeaderboardEnv(CarlaMultiAgentEnv):
    def __init__(
        ...
        all_tasks = self.build_all_tasks(
            carla_map, weather_group, routes_group, routes_file
        )
        super().__init__(
            carla_map,
            host,
    ...
    def build_all_tasks(carla_map, weather_group, routes_group, routes_file):
        ...
        if carla_map == "Town04" and routes_group is not None:
            description_folder = (
                CARLA_GYM_ROOT_DIR
                / "envs/scenario_descriptions/LeaderBoard"
        ...
        print(
            "description folder {}, actor config dict {}".format(
                description_folder, actor_configs_dict
            )
        )
        if routes_file is None:
            ...
            route_descriptions_dict = config_utils.parse_routes_file(
                description_folder / routes_file
            )
