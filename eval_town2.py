...
def evaluate_agent(ckpt_path):
    ...
    observation_space = {}
    observation_space["birdview"] = gym.spaces.Box(
        low=0, high=255, shape=(3, 192, 192), dtype=np.uint8
    )
    rgb_channels = 3
    rgb_space = gym.spaces.Box(
        low=0, high=255, shape=(rgb_channels, 144, 256), dtype=np.uint8
    )
    ...
    for route_id in tqdm.tqdm(range(6)):
        ...
        while not c_route and not done_ep:
            obs = raw_obs["hero"]
            obs = RlBirdviewWrapper.process_obs(
                obs, env_wrapper_configs["input_states"]
            )
            obs_dict = dict([(k, np.stack([obs[k]])) for k in obs])

            actions, _, _, _, _, _, fake_birdview = policy.forward(
            ...
            for i_mask in range(1):
                ...
                Image.fromarray(birdview_mask).save(
                    episode_dir
                    / "birdview"
                    / "{:0>4d}_{:0>2d}.png".format(i_step, i_mask)
                )
            ...
            Image.fromarray(central_rgb).save(
                episode_dir / "central_rgb" / "{:0>4d}.png".format(i_step)
            )
            left_rgb = obs["left_rgb"]
            left_rgb = np.transpose(left_rgb, [1, 2, 0]).astype(np.uint8)
            Image.fromarray(left_rgb).save(
                episode_dir / "left_rgb" / "{:0>4d}.png".format(i_step)
            )
            right_rgb = obs["right_rgb"]
            right_rgb = np.transpose(right_rgb, [1, 2, 0]).astype(np.uint8)
            Image.fromarray(right_rgb).save(
                episode_dir / "right_rgb" / "{:0>4d}.png".format(i_step)
            )

            traj_plot = obs["traj_plot"].numpy()
            traj_plot = np.transpose(traj_plot, [1, 2, 0]).astype(np.uint8)
            zero_img = np.zeros(
                [traj_plot.shape[0], traj_plot.shape[0], 3], dtype=np.uint8
            )
            zero_img[:, :, 0:1] = traj_plot
            Image.fromarray(zero_img).save(
                episode_dir / "traj_plot" / "{:0>4d}.png".format(i_step)
            )
            ...
            route_completed_in_m = info["hero"]["route_completion"][
                "route_completed_in_m"
            ]
    ...
    ep_df = pd.DataFrame({"routes_completed": routes_completed})

    ep_df.to_json(eval_file_dir / "eval.json")


if __name__ == "__main__":
    ckpt_file = pathlib.Path("ckpt/ckpt_73728.pth")
