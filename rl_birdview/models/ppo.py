...
class PPO:
    ...
    def collect_rollouts(
        ...
        if self.gail:
            ...
            for step in range(rollout_buffer.buffer_size):
                obs_dict = dict(
                    [
                        (obs_key, th.as_tensor(obs_item[step]))
                        for obs_key, obs_item in rollout_buffer.observations.items()
                    ]
                )
    ...
    def train(self):
        ...
        if self.policy.fake_birdview:
            data_loader_fake_birdview = (
                self.policy.gan_fake_birdview.fill_expert_dataset(
                    self.discriminator.expert_loader
                )
            )
        ...
        for epoch in range(self.n_epochs):
            print(
                "PPO Training epoch {} out of total epochs {}".format(
                    epoch, self.n_epochs
                )
            )
            approx_kl_divs = []
            ...
            for i in range(data_len):
                ...
                values, log_prob, entropy_loss, exploration_loss, distribution = (
                    self.policy.evaluate_actions(
                        rollout_data.observations,
                        rollout_data.actions,
                        rollout_data.exploration_suggests,
                        rollout_data.fake_birdviews,
                        detach_values=False,
                    )
                )
                # Normalize advantage
                ...
                for expert_batch in self.discriminator.expert_loader:
                    ...
                    fake_birdview = None
                    if self.policy.fake_birdview:
                        fake_birdview = data_loader_fake_birdview.index_select(
                            dim=0, index=expert_obs_dict["item_idx"].int()
                        )
                        fake_birdview = fake_birdview.to(self.policy.device)
                    expert_action = expert_action.to(self.policy.device)
                    # Get BC loss
                    alogprobs, _ = self.policy.evaluate_actions_bc(
                        obs_tensor_dict, fake_birdview, expert_action
                    )
                    bcloss = -alogprobs.mean()
                    # action loss is weighted sum
                    action_loss = (
                        self.gail_gamma * bcloss + (1 - self.gail_gamma) * policy_loss
                    )
    ...
    def train_gan(self):
        ...
        for _ in range(gan_epochs):
            for policy_batch in self.buffer.get(gan_batch_size):
                gan_disc_loss, gan_generator_loss, gan_pixel_loss, gan_loss = (
                    self.policy.gan_fake_birdview.train_batch(
                        policy_batch.observations, self.num_timesteps
                    )
                )
    ...
    def learn(self, total_timesteps, callback=None, seed=2021):
        ...
        while self.num_timesteps < total_timesteps:
            print(
                "=====> learning step {} out of total timestep {}".format(
