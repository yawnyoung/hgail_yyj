...
class RlBirdviewWrapper(gym.Wrapper):
    def __init__(
        self, env, input_states=[], acc_as_action=False, env_id=0, resume=False
    ):
        ...
        observation_space = {}
        observation_space["birdview"] = env.observation_space[self._ev_id]["birdview"][
            "masks"
        ]
        if "rgb" in input_states:
            rgb_height, rgb_width, _ = env.observation_space[self._ev_id][
                "central_rgb"
            ]["data"].shape
            rgb_channels = 3
            rgb_space = gym.spaces.Box(
                low=0,
                high=255,
                shape=(rgb_channels, rgb_height, rgb_width),
                dtype=np.uint8,
            )
        ...
        if "speed" in self._input_states:
            observation_space["speed"] = env.observation_space[self._ev_id]["speed"][
                "speed_xy"
            ]
        if "speed_limit" in self._input_states:
            observation_space["speed_limit"] = env.observation_space[self._ev_id][
                "control"
            ]["speed_limit"]
        if "control" in self._input_states:
            observation_space["control"] = gym.spaces.Box(
                low=-10.0, high=30.0, shape=(4,), dtype=np.float32
            )
        if "acc_xy" in self._input_states:
            observation_space["acc_xy"] = env.observation_space[self._ev_id][
                "velocity"
            ]["acc_xy"]
        if "vel_xy" in self._input_states:
            observation_space["vel_xy"] = env.observation_space[self._ev_id][
                "velocity"
            ]["vel_xy"]
        if "vel_ang_z" in self._input_states:
            observation_space["vel_ang_z"] = env.observation_space[self._ev_id][
                "velocity"
            ]["vel_ang_z"]
        if "linear_speed" in self._input_states:
            observation_space["linear_speed"] = gym.spaces.Box(
                low=-10.0, high=30.0, shape=(1,), dtype=np.float32
            )
        if "vec" in self._input_states:
            observation_space["vec"] = gym.spaces.Box(
                low=-10.0, high=30.0, shape=(2,), dtype=np.float32
            )
        if "traj" in self._input_states:
            observation_space["traj"] = gym.spaces.Box(
                low=-10.0, high=30.0, shape=(10,), dtype=np.float32
            )
            observation_space["traj_plot"] = gym.spaces.Box(
                low=0, high=255, shape=(1, 192, 192), dtype=np.uint8
            )
            observation_space["traj_plot_rgb"] = gym.spaces.Box(
                low=0, high=255, shape=(1, 144, 256), dtype=np.uint8
            )
        if "cmd" in self._input_states:
            observation_space["cmd"] = gym.spaces.Box(
                low=-10.0, high=30.0, shape=(6,), dtype=np.float32
            )
        if "command" in self._input_states:
            observation_space["command"] = gym.spaces.Box(
                low=-10.0, high=30.0, shape=(1,), dtype=np.float32
            )
        ...
        if self._acc_as_action:
            ...
            env.action_space = gym.spaces.Box(
                low=np.array([-0.35, -1]), high=np.array([0.8, 1]), dtype=np.float32
            )
        else:
        ...
        if resume:
            ep_df = pd.read_csv(
                self.output_path / "{}.csv".format(self._env_id), on_bad_lines="skip"
            )
    ...
    def reset_monitor(self):
        ...
        if self.ep_count < 2:
            ep_df.to_csv(self.output_path / "{}.csv".format(self._env_id), index=False)
        else:
            ep_df.to_csv(
    ...
    def process_obs(obs, input_states, train=True):
        ...
        if "vec" in input_states or "traj" in input_states:
            ...
            # imu nan bug
            compass = (
                0.0 if np.isnan(obs["gnss"]["imu"][-1]) else obs["gnss"]["imu"][-1]
            )
            ...
            while (point_idx + 1) * 3 <= traj_points.shape[0]:
                ...
                loc_in_ev = trans_utils.vec_global_to_ref(
        ...
        obs_dict.update({"birdview": birdview})
