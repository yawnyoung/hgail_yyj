...
env_eval_configs = {"weather_group": "simple", "routes_group": "train"}
...
multi_env_configs = [
    ...
    # {"host": "localhost", "port": 2008, "carla_map": "Town01"},
    # {"host": "localhost", "port": 2010, "carla_map": "Town01"},
]
...
multi_env_eval_configs = [
    {"host": "localhost", "port": 2012, "carla_map": "Town02"},
]
...
def get_env_wrapper_configs(rgb=True):
    ...
    if rgb:
        env_wrapper_configs["input_states"].extend(
            ["linear_speed", "vec", "cmd", "command", "traj", "rgb"]
        )
...
def env_maker(env_id, config, obs_configs, env_wrapper_configs, rendering=True):
    env = EndlessEnv(
        ...
        seed=2021,
        no_rendering=(not rendering),
        **env_configs,
        **config
    )
    env = RlBirdviewWrapper(
        env, **env_wrapper_configs, env_id=env_id, resume=RESUME_LAST_TRAIN
    )
    return env
...
if __name__ == "__main__":
    ...
    env = SubprocVecEnv(
        [
            lambda env_id=env_id, config=config: env_maker(
                env_id, config, obs_configs, env_wrapper_configs, rendering=generate_rgb
            )
            for env_id, config in enumerate(multi_env_configs)
        ]
    )
    env_eval = SubprocVecEnv(
        [
            lambda env_id=env_id, config=config: env_eval_maker(
                env_id, config, obs_configs, env_wrapper_configs, rendering=generate_rgb
            )
            for env_id, config in enumerate(multi_env_eval_configs)
        ]
    )
    if not RGB_GAIL:
    ...
    policy_kwargs = {
        ...
        "distribution_entry_point": "rl_birdview.models.distributions:SquashedDiagGaussianDistribution",
    ...
    if RESUME_LAST_TRAIN:
        ...
        all_ckpts = [
            ckpt_file
            for ckpt_file in wandb_run.files()
            if "ckpt_latest" in ckpt_file.name
        ]
    ...
    agent = PPO(policy=policy, discriminator=discriminator, env=env, **train_kwargs)
